...
export async function createExperimentInfoCard(experimentName: string): Promise<FrameNode> {
  ...
  // Description section
  const descSection = await createSection("Description", "Lorem ipsum");
  card.appendChild(descSection);
  // Hypothesis section
  const hypSection = await createSection("Hypothesis", "Lorem ipsum");
...
async function createSection(label: string, value: string): Promise<FrameNode> {
  // Ensure fonts are loaded before creating text nodes
  await loadFonts();
...
function createMetricRow(label: string, value: string): FrameNode {
...
function createLinkChip(label: string): FrameNode {
