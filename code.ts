...
if (figma.editorType === 'figma') {
  ...
  figma.ui.onmessage = async (msg: any) => {
    if (msg.type === 'create-flow' && msg.payload) {
      ...
      // --- Helper utilities ---
      function createPill(text: string, bgColor: RGB, textColor: RGB): FrameNode {
      ...
      function createVariantCard(variant: any): FrameNode {
        ...
        if (variant.status === 'Winner' || variant.status === 'Running') {
          const badge = createPill(
            variant.status,
            variant.status === 'Winner' ? WINNER_COLOR : RUNNING_COLOR,
            { r: 1, g: 1, b: 1 }
          );
      ...
      function createNodeCard(title: string, subtitle?: string, trafficLabel?: string): FrameNode {
        ...
        card.fills = [{ type: 'SOLID' as const, color: ENTRY_EXIT_CARD_BG.color }];
      ...
      function connectNodes(fromNode: SceneNode, toNode: SceneNode, options?: { winner?: boolean, label?: string }): ConnectorNode {
        ...
        connector.strokes = [{ type: 'SOLID', color: options?.winner ? WINNER_CONNECTOR_COLOR : CONNECTOR_COLOR }];
      ...
      roundContainer.fills = [{ type: 'SOLID' as const, color: VARIANT_CONTAINER_BG.color }];
