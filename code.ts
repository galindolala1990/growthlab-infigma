  ...
  figma.ui.onmessage = async (msg: PluginMessage) => {
    if (msg.type === 'create-flow' && msg.payload) {
      ...
      async function loadFonts() {
        ...
        await figma.loadFontAsync({ family: "Figtree", style: "Regular", weight: 400 }).catch(()=>{});
        // Try to load Semibold 600, fallback to Medium 600 if not available
        try {
          await figma.loadFontAsync({ family: "Figtree", style: "Semibold", weight: 600 });
        } catch {
          await figma.loadFontAsync({ family: "Figtree", style: "Medium", weight: 600 }).catch(()=>{});
        }
        // Always load Figtree Bold 700
        await figma.loadFontAsync({ family: "Figtree", style: "Bold", weight: 700 }).catch(()=>{});
